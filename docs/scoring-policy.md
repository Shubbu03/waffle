...
The current repository has the pure policy, scorer, trade gate, and tests. The watcher has not yet fetched mint/pool/optional evidence or persisted scored signals; the mobile app and API have not yet wired this gate into trade actions. When C1.3 and C3 are implemented, record actual score distributions and rejection reasons, then version any policy change rather than editing v1 in place. Device and mainnet behavior require the user's manual verification under the project workflow.
