...
Issue #13: confirmed PumpSwap buys now collect cached exact-mint state, verified pool/vault reserves with fresh SOL/USD valuation, and configured-size Jupiter quote probes. Optional holder/creator evidence is explicitly unknown without an authoritative source; only fresh configured same-asset Pyth feeds produce oracle evidence. Automated boundary/cache/freshness tests pass. Live provider verification and scored-signal persistence remain pending.
