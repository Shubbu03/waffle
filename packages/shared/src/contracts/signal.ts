...
export const signalDetailSchema = signalSummarySchema.extend({
  ...
  for (const [index, group] of SCORE_REASON_GROUPS.entries()) {
    ...
    const matches = reason.code === group.pass || reason.code === group.fail ||
    ...
    if (!matches || reason.points !== expectedPoints) {
      context.addIssue({ code: "custom", path: ["reasons", index], message: "Reason code or points do not match score v1" });
    }
  }
  ...
  const suppressed = [
    "unsupported_or_failed_transaction", "mint_unavailable_or_unsafe",
    "pool_unavailable_or_shallow", "quote_unavailable", "oracle_deviation_high",
  ...
  if (codes.has("mint_safe") && (
    snapshot.mint === null || snapshot.mint.address !== signal.mintAddress ||
    snapshot.mint.tokenProgramId !== SPL_TOKEN_PROGRAM_ID ||
    snapshot.mint.mintAuthority !== null || snapshot.mint.freezeAuthority !== null
  )) {
    context.addIssue({ code: "custom", path: ["snapshot", "mint"], message: "Mint evidence does not support the awarded points" });
  }
  if (codes.has("pool_liquid") && (
    snapshot.pool === null || snapshot.pool.baseMint !== signal.mintAddress ||
    snapshot.pool.quoteMint !== WRAPPED_SOL_MINT ||
    snapshot.pool.liquidityUsd < scorePolicyV1.liquidityUsd.signalAndPaper
  )) {
    context.addIssue({ code: "custom", path: ["snapshot", "pool"], message: "Pool evidence does not support the awarded points" });
  }
  if (codes.has("quote_available") && (
    snapshot.quote === null || snapshot.quote.inputMint !== WRAPPED_SOL_MINT ||
    ...
    context.addIssue({ code: "custom", path: ["snapshot", "quote"], message: "Quote evidence does not support the awarded points" });
  }
  if (codes.has("holders_acceptable") && (
    snapshot.holders === null || snapshot.holders.top10Pct > scorePolicyV1.optional.maxTop10HolderPct
  )) {
    context.addIssue({ code: "custom", path: ["snapshot", "holders"], message: "Holder evidence does not support the awarded points" });
  }
  if (codes.has("creator_acceptable") && (
    snapshot.creator === null || snapshot.creator.holdingPct > scorePolicyV1.optional.maxCreatorHoldingPct
  )) {
    context.addIssue({ code: "custom", path: ["snapshot", "creator"], message: "Creator evidence does not support the awarded points" });
  }
  if (codes.has("oracle_agrees") && (
    snapshot.oracle === null || snapshot.oracle.mintAddress !== signal.mintAddress ||
    snapshot.oracle.deviationBps > scorePolicyV1.optional.maxOracleDeviationBpsForPoints
  )) {
    context.addIssue({ code: "custom", path: ["snapshot", "oracle"], message: "Oracle evidence does not support the awarded points" });
  }
});
