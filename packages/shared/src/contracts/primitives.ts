...
export const solanaAddressSchema = z.string().refine(
  (value) => isBase58Bytes(value, 32),
  "Expected a 32-byte base58 Solana address",
);

export const transactionSignatureSchema = z.string().refine(
  (value) => isBase58Bytes(value, 64),
  "Expected a 64-byte base58 transaction signature",
);
...
export const rawAmountSchema = z.string().max(20).refine(
  (value) => {
...
export const eventCursorSchema = z.string().max(19).refine(
  (value) => {
