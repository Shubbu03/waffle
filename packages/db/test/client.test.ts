...
test("API login excludes owner and bypass privileges", () => {
  const restricted = {
    canLogin: true, superuser: false,
    bypassRls: false, createRole: false, ownsAppTables: false,
    apiRoleUsable: true, watcherMember: false, deliveryMember: false,
  };
  expect(isRestrictedApiLogin(restricted)).toBe(true);
  for (const flag of ["superuser", "bypassRls", "createRole", "ownsAppTables", "watcherMember", "deliveryMember"] as const) {
