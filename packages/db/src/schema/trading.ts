...
import { bigint, boolean, check, index, integer, jsonb, pgTable, timestamp, uniqueIndex, uuid, varchar } from "drizzle-orm/pg-core";
...
export const paperPositions = pgTable("paper_positions", {
  ...
  signalId: uuid("signal_id").notNull().references(() => signals.id, { onDelete: "restrict" }),
  ...
  check("paper_positions_status_check", sql`(${table.status} = 'open' AND ${table.closedAt} IS NULL) OR (${table.status} = 'closed' AND ${table.closedAt} IS NOT NULL)`),
]);

export const tradeAttempts = pgTable("trade_attempts", {
  id: uuid("id").defaultRandom().primaryKey(),
  userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  signalId: uuid("signal_id").notNull().references(() => signals.id, { onDelete: "restrict" }),
  ...
  check("trade_attempts_size_check", sql`${table.inputAmountLamports} > 0 AND ${table.inputAmountLamports} <= 50000000`),
  check("trade_attempts_router_check", sql`${table.router} IN ('metis', 'dflow', 'okx')`),
  check("trade_attempts_status_check", sql`${table.status} IN ('prepared', 'wallet_rejected', 'submitted', 'confirmed', 'failed')`),
  check("trade_attempts_confirmed_check", sql`${table.status} <> 'confirmed' OR (${table.signature} IS NOT NULL AND ${table.executeCode} = 0 AND ${table.failureReason} IS NULL)`),
]);
