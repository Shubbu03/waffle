...
export const authChallenges = pgTable("auth_challenges", {
  ...
  check("auth_challenges_expiry_check", sql`${table.expiresAt} > ${table.issuedAt} AND ${table.expiresAt} <= ${table.issuedAt} + interval '5 minutes'`),
...
export const sessions = pgTable("sessions", {
  ...
  check("sessions_expiry_check", sql`${table.expiresAt} > ${table.createdAt} AND ${table.expiresAt} <= ${table.createdAt} + interval '7 days'`),
]);
