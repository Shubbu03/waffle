...
import { boolean, check, index, pgTable, primaryKey, text, timestamp, uniqueIndex, uuid, varchar } from "drizzle-orm/pg-core";
...
export const userWalletSubscriptions = pgTable("user_wallet_subscriptions", {
  userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  watchedWalletId: uuid("watched_wallet_id").notNull().references(() => watchedWallets.id, { onDelete: "restrict" }),
  ...
  check("user_wallet_subscriptions_alert_timestamp_check", sql`${table.alertsEnabled} = (${table.alertsEnabledAt} IS NOT NULL)`),
]);
