...
export type JupiterServiceErrorCode =
  | "INVALID_REQUEST" | "INVALID_ORDER" | "STALE_ORDER" | "UNSUPPORTED_ORDER"
  | "UPSTREAM_UNAVAILABLE" | "UPSTREAM_INVALID" | "EXECUTION_UNKNOWN";

export class JupiterServiceError extends Error {
  constructor(readonly code: JupiterServiceErrorCode, readonly upstreamStatus?: number) {
    super(code);
...
export class JupiterService {
  ...
  async getPaperQuote(input: unknown): Promise<PaperQuote> {
    ...
    if (response.inputMint !== WRAPPED_SOL_MINT || response.outputMint !== request.outputMint ||
      response.inAmount !== request.inputAmountLamports || response.taker != null || response.transaction !== null) {
  ...
  async getRealOrder(input: unknown, connectedWallet: string): Promise<RealOrder> {
    ...
    if (!parsed.success || !solanaAddressSchema.safeParse(connectedWallet).success ||
      parsed.data.taker !== connectedWallet) throw new JupiterServiceError("INVALID_REQUEST");
    const request = parsed.data;
    const requestedAt = this.now();
    const response = await this.fetchOrder(request.outputMint, request.inputAmountLamports, request.taker);
    if (response.inputMint !== WRAPPED_SOL_MINT || response.outputMint !== request.outputMint ||
      response.inAmount !== request.inputAmountLamports || response.taker !== request.taker) {
      throw new JupiterServiceError("UPSTREAM_INVALID");
    }
    if (response.router === "jupiterz" || response.signatureFeePayer !== request.taker ||
      response.gasless === true || !response.transaction || !response.lastValidBlockHeight) {
  ...
  async execute(input: unknown, connectedWallet: string): Promise<JupiterExecutionResult> {
    ...
    if (transaction.version !== 0 || transaction.message.header.numRequiredSignatures !== 1 ||
      transaction.signatures.length !== 1 || !signature ||
    ...
    let payload: unknown;
    try {
      payload = await this.fetchJson(`${BASE_URL}/execute`, {
        method: "POST",
    ...
    if (data.status === "Success" && data.code === 0 && parsedSignature.success &&
      data.totalInputAmount && data.totalOutputAmount && data.inputAmountResult && data.outputAmountResult) {
      return jupiterExecutionResultSchema.parse({
        status: "confirmed", requestId: request.requestId, code: 0, signature: parsedSignature.data,
        totalInputAmountRaw: data.totalInputAmount, totalOutputAmountRaw: data.totalOutputAmount,
        inputAmountResultRaw: data.inputAmountResult, outputAmountResultRaw: data.outputAmountResult,
      });
    }
    if (data.status === "Failed" && data.code < 0) {
      return jupiterExecutionResultSchema.parse({
        status: "failed", requestId: request.requestId, code: data.code,
  ...
  private commonQuote(response: z.infer<typeof orderResponseSchema>, signalId: string, requestedAt: number) {
    const expiresAt = Math.min(requestedAt + scorePolicyV1.freshness.quoteMs,
    ...
    return {
      id: crypto.randomUUID(), signalId,
      inputMint: response.inputMint, outputMint: response.outputMint,
      inputAmountLamports: response.inAmount, outputAmountRaw: response.outAmount,
      minOutputAmountRaw: response.otherAmountThreshold,
      requestId: response.requestId, router: response.router,
      feeLamports: (signature + priority + rent).toString(),
      fees: {
        totalBps: response.feeBps, mint: response.feeMint ?? null,
        ...
        signatureLamports: signature.toString(), prioritizationLamports: priority.toString(), rentLamports: rent.toString(),
      },
      ...
      fetchedAt: new Date(requestedAt).toISOString(), expiresAt: new Date(expiresAt).toISOString(),
    };
  }
  ...
  private async fetchOrder(outputMint: string, amount: string, taker?: string) {
    ...
    const payload = await this.fetchJson(url, {
      headers: { "x-api-key": this.apiKey }, signal: AbortSignal.timeout(8_000),
    });
  ...
  private async fetchJson(url: URL | string, init: RequestInit, execution = false): Promise<unknown> {
    ...
    if (!response.ok) throw new JupiterServiceError(execution ? "EXECUTION_UNKNOWN" : "UPSTREAM_UNAVAILABLE", response.status);
    let body: string;
    try {
      body = await response.text();
      if (body.length > MAX_RESPONSE_BYTES) throw new Error("oversized response");
  ...
  private decodeTransaction(encoded: string, errorCode: "INVALID_ORDER" | "UNSUPPORTED_ORDER" = "UNSUPPORTED_ORDER"): VersionedTransaction {
    try {
      const bytes = Buffer.from(encoded, "base64");
      if (bytes.length > 1232 || bytes.length === 0 || bytes.toString("base64") !== encoded) throw new Error("invalid bytes");
  ...
  private assertResponseFresh(response: z.infer<typeof orderResponseSchema>, requestedAt: number): void {
    if (this.now() >= requestedAt + scorePolicyV1.freshness.quoteMs ||
  ...
  private remember<T extends { expiresAt: string }>(cache: Map<string, T>, id: string, value: T): void {
    ...
    if (cache.size >= MAX_PENDING) cache.delete(cache.keys().next().value!);
    cache.set(id, value);
  }
}
