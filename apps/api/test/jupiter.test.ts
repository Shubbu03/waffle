...
const message = new TransactionMessage({
  ...
  instructions: [SystemProgram.transfer({ fromPubkey: wallet.publicKey, toPubkey: otherWallet.publicKey, lamports: 1 })],
}).compileToV0Message();
...
const changed = new VersionedTransaction(new TransactionMessage({
  payerKey: wallet.publicKey,
  recentBlockhash: "11111111111111111111111111111111",
  instructions: [SystemProgram.transfer({ fromPubkey: wallet.publicKey, toPubkey: otherWallet.publicKey, lamports: 2 })],
}).compileToV0Message());
changed.sign([wallet]);
...
const realRequest = {
  signalId, outputMint: PUMP_SWAP_PROGRAM_ID,
  inputAmountLamports: "50000000", taker: wallet.publicKey.toBase58(),
};
...
describe("Jupiter quote and order service", () => {
  test("paper quote omits taker and transaction, retains fee and route evidence, and expires locally", async () => {
    ...
    const service = mockService(upstreamOrder({ taker: null, transaction: null, quoteId: "provider-quote" }), requests, () => clock);
    const quote = await service.getPaperQuote({ signalId, outputMint: PUMP_SWAP_PROGRAM_ID, inputAmountLamports: "50000000" });
    expect(requests).toHaveLength(1);
    expect(requests[0]!.url.searchParams.has("taker")).toBe(false);
    expect(requests[0]!.url.searchParams.get("inputMint")).toBe(WRAPPED_SOL_MINT);
    expect((requests[0]!.init.headers as Record<string, string>)["x-api-key"]).toBe("server-secret");
    expect(quote).toMatchObject({
      kind: "paper", requestId: "jup-request-1", router: "metis",
      providerQuoteId: "provider-quote", minOutputAmountRaw: "950",
      feeLamports: "6000", fees: { totalBps: 10, signatureLamports: "5000", prioritizationLamports: "1000" },
      priceImpactPct: -0.5, priceImpactBps: 50,
    });
  ...
  test("rejects invalid, stale, mismatched, and transaction-bearing paper quotes", async () => {
    ...
    await expect(service.getPaperQuote({ ...request, inputAmountLamports: "100000001" })).rejects.toMatchObject({ code: "INVALID_REQUEST" });
  ...
  test("real order requests one-signature non-sponsored routing and preserves fee payers", async () => {
    ...
    expect(order).toMatchObject({
      kind: "real", requiredSignatures: 1, gasless: false,
      signatureFeePayer: wallet.publicKey.toBase58(),
      prioritizationFeePayer: wallet.publicKey.toBase58(),
      rentFeePayer: wallet.publicKey.toBase58(),
      lastValidBlockHeight: "123456789", requestId: "jup-request-1",
    });
  });
  ...
  test("rejects unsupported routes, fee sponsorship, malformed signatures, and wallet mismatch", async () => {
    for (const [payload, code] of [
      ...
      await expect(mockService(payload).getRealOrder(realRequest, wallet.publicKey.toBase58())).rejects.toMatchObject({ code });
    }
    await expect(mockService(upstreamOrder()).getRealOrder(realRequest, otherWallet.publicKey.toBase58())).rejects.toMatchObject({ code: "INVALID_REQUEST" });
    await expect(mockService(upstreamOrder()).getRealOrder({ ...realRequest, inputAmountLamports: "50000001" }, wallet.publicKey.toBase58())).rejects.toMatchObject({ code: "INVALID_REQUEST" });
    await expect(mockService(upstreamOrder({ expireAt: new Date(now - 1).toISOString() })).getRealOrder(realRequest, wallet.publicKey.toBase58())).rejects.toMatchObject({ code: "STALE_ORDER" });
  });
  ...
  test("executes only the exact signed order once, without calling a live endpoint", async () => {
    ...
    const service = mockService(upstreamOrder(), requests, () => now, {
      status: "Success", code: 0, signature: "1".repeat(64),
      totalInputAmount: "50000000", totalOutputAmount: "990",
      inputAmountResult: "50000000", outputAmountResult: "1000",
    });
    const order = await service.getRealOrder(realRequest, wallet.publicKey.toBase58());
    const execute = { orderId: order.id, requestId: order.requestId, signedTransactionBase64: signedBase64 };
    await expect(service.execute({ ...execute, requestId: "wrong" }, wallet.publicKey.toBase58())).rejects.toMatchObject({ code: "INVALID_ORDER" });
    await expect(service.execute({ ...execute, signedTransactionBase64: unsignedBase64 }, wallet.publicKey.toBase58())).rejects.toMatchObject({ code: "INVALID_ORDER" });
    await expect(service.execute({ ...execute, signedTransactionBase64: changedBase64 }, wallet.publicKey.toBase58())).rejects.toMatchObject({ code: "INVALID_ORDER" });
    await expect(service.execute(execute, otherWallet.publicKey.toBase58())).rejects.toMatchObject({ code: "INVALID_ORDER" });
    expect(requests).toHaveLength(1);
    expect(await service.execute(execute, wallet.publicKey.toBase58())).toMatchObject({
      status: "confirmed", code: 0, totalOutputAmountRaw: "990", outputAmountResultRaw: "1000",
    });
    expect(requests).toHaveLength(2);
    expect(JSON.parse(requests[1]!.init.body as string)).toEqual({
      signedTransaction: signedBase64, requestId: order.requestId, lastValidBlockHeight: order.lastValidBlockHeight,
    });
    await expect(service.execute(execute, wallet.publicKey.toBase58())).rejects.toMatchObject({ code: "INVALID_ORDER" });
    expect(requests).toHaveLength(2);
  });

  test("reports upstream failures and uncertain execution without retrying", async () => {
    const request = { signalId, outputMint: PUMP_SWAP_PROGRAM_ID, inputAmountLamports: "50000000" };
    const unavailable = new JupiterService("server-secret", (async () => new Response("rate limited", { status: 429 })) as unknown as typeof fetch, () => now);
    await expect(unavailable.getPaperQuote(request)).rejects.toMatchObject({ code: "UPSTREAM_UNAVAILABLE", upstreamStatus: 429 });
    ...
    const requests: Array<{ url: URL; init: RequestInit }> = [];
    const service = mockService(upstreamOrder(), requests, () => now, { status: "Failed", code: -1000, signature: null });
    const order = await service.getRealOrder(realRequest, wallet.publicKey.toBase58());
    expect(await service.execute({ orderId: order.id, requestId: order.requestId, signedTransactionBase64: signedBase64 }, wallet.publicKey.toBase58())).toMatchObject({
      status: "failed", code: -1000, signature: null,
    });
    const failedFetch = new JupiterService("server-secret", (async (url) => {
    ...
    const attempt = { orderId: uncertainOrder.id, requestId: uncertainOrder.requestId, signedTransactionBase64: signedBase64 };
    await expect(failedFetch.execute(attempt, wallet.publicKey.toBase58())).rejects.toMatchObject({ code: "EXECUTION_UNKNOWN" });
    await expect(failedFetch.execute(attempt, wallet.publicKey.toBase58())).rejects.toMatchObject({ code: "INVALID_ORDER" });
  });
});
